...
class HomeController extends Controller
{
    ...
    public function getMyTables(){
        ...
        return $this->index($this->getDotEnv('DB_DATABASE'), 'tablestrack');
    }
    ...
    public function getTablesInTableGroups($db_name, $table_group)
    {
        \Log::Info("getTableGroups");
        $this->hiddens = array('table_group_name', 'db_name', 'table_description', 'keywords', 'private', 'field_indexes', 
            'fillable', 'hidden', 'created_by', 'created_at', 'updated_at', 'id');
        $this->filters = array('db_name' => $db_name, 'table_group_name' => $table_group);
        return $this->index($this->getDotEnv('DB_DATABASE'), 'tablestrack');
    }

    public function getTableFields($db_name, $table)
    {
        \Log::Info("getTableFields");
        $this->setTable("tablestrack");
        $record = $this->table::where('table_name', $table)->first();
        $field_indexes = unserialize( base64_decode( $record->field_indexes ) );
        $this->setTable("tablefields");
        $this->table = $this->table::all();
        $result = array();
    ...
    public function getTableDetails($db_name, $table)
    {
    ...
    public function getTableFieldDataTypes($db_name, $table)
    {
        \Log::Info("getTableDetails");
        $this->setTable("tablestrack");
        $record = $this->table::where('table_name', $table)->first();
    ...
    public function createTable()
    {
        ...
        Schema::create($newtable, function (Blueprint $table) use ($fields, $composite_indexes) {
            $table->increments('id');
                foreach ($fields as $key => $value) {
        ...
        $myfile = fopen(app_path() ."/Models/" . ucwords(rtrim($newtable,'s')) .".php", "w");
        $line1 = "<?php\n";
    ...
    public function updateTable()
    {
        ...
        $this->setTable("tablestrack");
        $record = $this->table::where('table_name', $newtable)->update([
            "table_description" => $table_description,
            "keywords" => $keywords,
            "field_indexes" => base64_encode( serialize( $field_indexes ) ),
            "fillable" => base64_encode( serialize( $fillable ) ),
        ...
        Schema::table($newtable, function (Blueprint $table) use ($fields, $composite_indexes, $delete_fields) {
